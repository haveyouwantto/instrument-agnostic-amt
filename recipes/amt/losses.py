...
def _compute_v2_losses(
    ...
    *,
    ...
    pair_gate_logits = outputs["pair_gate_logits"]
    pitch_interval_query = outputs["pitch_interval_query"]
    pitch_interval_key = outputs["pitch_interval_key"]
    pitch_interval_diag = outputs["pitch_interval_diag"]
    instrument_interval_query = outputs["instrument_interval_query"]
    instrument_interval_key = outputs["instrument_interval_key"]
    instrument_interval_diag = outputs["instrument_interval_diag"]
    ...
    selected_pair_ids, flat_targets = _select_pair_candidates(
        pair_gate_logits,
        interval_targets,
        train_topk=config.instrument_pair_train_topk,
        random_negatives=config.instrument_pair_random_negatives,
        max_pairs=config.instrument_pair_max_pairs,
    )
    ...
    total_loss = (
        semi_crf_loss * config.semi_crf_loss_weight
        + pair_gate_loss * config.pair_gate_loss_weight
    )
...
def compute_losses(
    ...
    *,
    ...
    if model.semi_crf_version == "v1":
        return compute_v1_losses(outputs, batch, config=config, model=model)
    return _compute_v2_losses(outputs, batch, config=config, model=model)
